	...
	execute( options = {} ) {
		...
		model.change( writer => {
			...
			for ( const element of blocks.reverse() ) {
				...
				// Turning on and the element is not a list item - it should get the full set of necessary attributes.
				else if ( !turnOff && !element.hasAttribute( 'listType' ) ) {
				...
				// Turning on and the element is already a list items but has different type - change it's type and
				// type of it's all siblings that have same indent.
				else if ( !turnOff && element.hasAttribute( 'listType' ) && element.getAttribute( 'listType' ) != this.type ) {
					writer.setAttributes( {
						listType: this.type
					}, element );
				}
